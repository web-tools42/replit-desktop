...
class DiscordHandler {
    ...
    connectDiscord() {
        if (!this.client)
            this.client = new discord_rpc.Client({ transport: 'ipc' });
        this.client.login({ clientId: '498635999274991626' }).catch((error) => {
            console.error(error);
        ...
        this.client.on('ready', () => {
            ...
            this.discordTimer = setInterval(() => {
                this.setPlayingDiscord().catch((e) =>
    ...
    async setPlayingDiscord() {
        let url = common.getUrl(this.window);
    ...
    async setTalkBoard(spliturl, windowObj) {
        ...
        return { viewing: viewing, talkBoard: common.capitalize(spliturl[1]) };
    }
    async setEditing(windowObj) {
        const { activeFile, plugins } = JSON.parse(
            await windowObj.webContents.executeJavaScript(
                'JSON.stringify(window.store.getState())'
            )
        );
        const replType = await windowObj.webContents.executeJavaScript(
            'document.querySelector("img.jsx-2652062152").title'
        );
        return {
            fileName: activeFile,
            largeImageKey: languages.displayNameToIcon[replType],
