...
requests.get('https://repl.it/talk/all', function (error, response, body) {
    html = editHtml(body);
...
requests.get(
    'https://repl.it/talk/share/Replit-site-wide-dark-theme/6198',
...
app.get('/custom', (req, res) => {
    ...
    if (query.graylogo == '') query.graylogo = 'on';
...
const publicThemes = {
    ...
    'Bluer Blue':
        'text=c0c0c0&text2=adadad&text3=a8a8a8&bg=10182b&main=0d101e&upvote=&graylogo=on&announcements=0d101e',
    'Solarized Dark':
        'text=839496&text2=819090&text3=839496&bg=002b36&main=073642&upvote=586e75&fallback=586e75&announcements=073642&announcements=073642',
    'Black Contrast':
        'text=fff&text2=fff&text3=fff&bg=000&main=000&upvote=fff&graylogo=on&announcements=000',
    Volcano:
        'text=ffff64&text2=ffd932&text3=ffd932&bg=2c0400&main=f05b00&upvote=ffe93e&graylogo=on&announcements=f05b00',
    'Actually Bad':
        'text=0f0&text2=fff&text3=fff&bg=f00&main=00f&upvote=fff&fallback=ff0&graylogo=off&announcements=00f',
    'Purple Night':
        'text=d321b2&text2=900376&text3=611d55&bg=231e22&main=393c42&upvote=f848d8&fallback=ac2493&announcements=393c42',
    'Mint Breeze':
        'text=b4f7f4&text2=8ece44&text3=358c07&bg=10da82&main=7cda10&upvote=358c07&announcements=7cda10&fallback=',
    Monokai:
        'text=ffd866&text2=fc9867&text3=ab9df2&bg=2c292d&main=2c292d&upvote=a9dc76&fallback=78dce8&announcements=2c292d',
    'Grayscale (special)': 'grayscale=true',
    'Rainbow [Epilepsy warning] (special)': 'rainbow=true',
    'Timchen (special)':
        'timchen=true&text=fff&upvote=fff&main=000&bg=000&announcements=000',
    'Pewdiepie (special)':
        'pewdiepie=true&text=fff&upvote=fff&main=000&bg=000&announcements=000',
    'Terminal (special)':
        'monospace=true&text=00aa00&text2=00ff00&text3=00aa00&bg=000000&main=111111&upvote=005500&announcements=121212'
};
...
app.get('/api/themes', (req, res) => {
    ...
    for (const themeName in publicThemes) {
        ...
        themesList.push({
            ...
            id: i,
            cssUrl:
                'https://darktheme.matdoes.dev/theme.css?' +
                publicThemes[themeName],
            installUrl:
                'https://darktheme.matdoes.dev/theme.user.js?' +
                publicThemes[themeName],
            previewUrl:
                'https://darktheme.matdoes.dev/proxy/talk/all?' +
                publicThemes[themeName],
...
app.get('/preview', (req, res) => {
    ...
    generateCss(req.query, function (rendered_css) {
        var html = global.cached_repltalk.replace(
            /<\/head>/gi,
            `<style>${rendered_css}</style></head>`
        );
        res.send(html);
    });
});
app.get('/preview-post', (req, res) => {
    generateCss(req.query, function (rendered_css) {
        var html = global.cached_repltalk_post.replace(
            /<\/head>/gi,
            `<style>${rendered_css}</style></head>`
        );
        res.send(html);
...
app.get('/proxy/*', (req, res) => {
    requests.get('https://repl.it/' + req.url.slice(7), function (
        error,
        response,
        body
    ) {
