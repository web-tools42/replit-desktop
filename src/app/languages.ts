...
const languages: LangDict = {
    ...
    'cpp': /\.(cats|idc|w|nc|upc|xpm|c[+px]{2}|cc|h[+px]{2}|[it]pp|tcc|inl)$/i,
    // 'crystal': /\.e?cr$/i,
    'csharp': /\.(cs|csx)$/i, //done
    // 'cshtml': /\.cshtml$/i,
    'web_project': /\.(css|x?html)$/i,
    // 'cssmap': /\.css\.map$/i,
    // 'cuda': /\.cu$/i,
    // 'd': /\.di?$/i,
    'dart': /\.dart$/i,
    ...
    'kotlin': /\.(kt|ktm|kts)$/i,
    'less': /\.less/i,
    'lisp': /\.(lsp|lisp|l|nl|ny|podsl|sexp)$/i,
    'livescript': /\.ls$/i,
    'log': /\.log$/i,
    'lua': /\.(lua|pd_lua|rbxs|wlua)$/i,
    'makefile': /(makefile|mk\.config|\.(mk|mak|make)|^BSDmakefile|^GNUmakefile|^makefile\.sco|^Kbuild|^mkfile|^\.?qmake)$/i,
    ...
    'perl': /\.(p(er)?l|al|ph|plx|pm|(psgi|xs)|pl6|[tp]6$|\.6pl|(pm6|p6m)|6pm|nqp|p6l|pod6|Rexfile)$/i,
